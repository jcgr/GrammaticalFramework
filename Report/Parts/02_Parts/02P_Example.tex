...
According to the \note{insert reference to deyoung-schuuermann-voteid2011}, translating the legal text into linear logic results in this formula (how linear logic works is explained in section \ref{LL}):
...
$\bang$ (S $\geq$ 1) \\
\pushRight{5}{$\lolli$ \{ counted-ballot(C, L) $\tensor$ \\}
\pushRight{13}{$\bang$elected(C) $\tensor$ \\}
\pushRight{13}{tally-votes(S-1, H-1, U-1) \}\\}}
\end{texto}
