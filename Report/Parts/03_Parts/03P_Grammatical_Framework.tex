...
\lstCode{A simple abstract syntax.}{GF_C01}
\begin{lstgf}
...
In the abstract syntax shown in \refCode{GF_C01}, there are three categories: \testCode{Formula}, \testCode{Connective} and \testCode{Ident}. Furthermore, there are three functions. The first function says that \testCode{\_VoteCard} and \testCode{\_BlankBallot} are of the type \testCode{Ident}. The second function says that \testCode{\_Lolli} is of the type \testCode{Connective}. The last function says that \testCode{\_Formula} takes three arguments (an \testCode{Ident}, a \testCode{Connective} and an \testCode{Ident}) and returns something of the type \testCode{Formula}.
...
\lstCode{A concrete implementation of the abstract syntax from \refCode{GF_C01} that understands linear logic.}{GF_C02}
\begin{lstgf}
...
In \refCode{GF_C01} \testCode{Formula}, \testCode{Connective} and \testCode{Ident} have been defined as records that can hold a \testCode{Str} (a string). \testCode{\_VoteCard}, \testCode{\_BlankBallot } and \testCode{\_Lolli} corresponds to a certain string. 
...
\lstCode{A concrete implementation of the abstract syntax from \refCode{GF_C01} that understands English.}{GF_C03}
\begin{lstgf}
...
With another concrete implementation (such as the English one in \refCode{GF_C03}), the program will be able to translate sentences from one language into the other, as long as the sentences adhere to the structure set by the abstract syntax. Or in this case, translate linear logic into English or the other way around.
...
\lstCode{A simple operation in GF that concatenates two strings.}{GF_C04}
\begin{lstgf}
oper 
...
\lstCode{A simple resource module.}{GF_C05}
\begin{lstgf}
resource StringOper = {
    oper
        SS : Type = {s : Str} ;
...
\lstCode{Using the resource module.}{GF_C06}
\begin{lstgf}
...
\lstCode{Defining a parameter for singular and plural versions of a word.}{GF_C07}
\begin{lstgf}
param 
    Number = Sg | Pl ;
\end{lstgf}
...
\lstCode{Telling the \testCode{lincat} that it takes a parameter and generates a Str.}{GF_C08}
\begin{lstgf}
lincat 
    Recipient = {s : Number => Str} ;
\end{lstgf}

The last thing that needs to be changed is the \testCode{lin} of the category that has been changed. Tables are used for this purpose. A table holds the different possibilities for the parameter, along with what is returned based on the parameter. \refCode{GF_C08} shows a table for \_VoteCard.

\lstCode{A table for \_VoteCard that returns the singular or plural version of the word depending on the parameter.}{GF_C09}
\begin{lstgf}
lin 
    _VoteCard = {
        s = table {
...
\refCode{GF_C09} shows how the parameters are used in the ConcreteEnglish program. Assuming i1 is \testCode{\_VoteCard}, and i2 is \testCode{\_BlankBallot}, FormulaSg creates a string saying "if i give an authorization card then i get a blank ballot" where line 2 creates a string saying "if i give multiple authorization cards i get multiple blank ballots".

\lstCode{Giving the parameter as an argument.}{GF_C10}
\begin{lstgf}
lin
