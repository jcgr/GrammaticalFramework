...
The types at the end of section \ref{LL_01} immediately pose a problem. As each connective gives an A, that A can be used in another connective. In theory, one could have a \formula{$\bang$$\bang$$\bang$voting-auth-card}, which does not make sense. They need to be split up to prevent this from happening.
