...
Celf has its own syntax for the connectives of linear logic, which will be used as the standard for the GF program. Furthermore, some operations (such as minus and equality operators) are different in Celf as well. The differences are the following:
\begin{texto}
	$\tensor$\hspace{40pt} ::= * \\
	$\lolli$\hspace{40pt}::= -o \\
	...
	$\lbrack$ x $|$ y$\rbrack$ \hspace{15pt} ::= (cons x y)
\end{texto}
This can be used to translate linear logic formulas into something a program can read more easily. The example from section \ref{E} therefore becomes:
\begin{texto}
