...
\note{Rewrite this section}
To assign a logic programming interpretation, restrictions must be placed on the ways in which these connectives fit together so that proof search becomes more deterministic. Using the Celf \note{reference needed} framework, the formulas are split into positive and negative classes, along with a monad that is used to prevent interference between them. As only few of linear logic’s connectives are used during this project, only some fragments of polarized monadic linear logic is needed:

\centered{
Negative Formulas A\super{-}, B\super{-} ::= P\super{-} $|$ $\forall$x:r. A\super{-} $|$ A\super{+} $\lolli$ \{ B\super{+} \}\\
Positive Formulas A\super{+}, B\super{+} ::= A\super{+} $\tensor$ B\super{+} $|$ $\one$ $|$ $\bang$A\super{-} $|$ A\super{-}}
...
\note{Go more in-depth with how it works?}
