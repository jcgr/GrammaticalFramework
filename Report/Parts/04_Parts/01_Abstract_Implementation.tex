...
\end{lstgf}
Going through the abstract syntax, one will notice two things that do not stem from the connectives: There is something called an \cf{"Atomic"} (line 13) and the \cf{\_Pi} has been split into mutiple parts. The \cf{Atomic} is used to represent "variables/functions(?)" along with the arguments they take. They will be explained later in this section. \\
The universal quantification (\cf{\_Pi}) has been split into three parts: One where it takes an ArgColl (more about that later) and produces a \cf{Pi}, one where it takes two \cf{Pi}s as arguments and produces a \cf{Pi} and one where it takes a \cf{Pi} and produces a \cf{Neg}. This allows multiple universan quantifiers to follow each other
